...
int main() {
    ...
    // Load templates from CSV
    std::vector<Template> templates = load_templates_from_csv("C:/thinning_cpu/build/templates_load_255.csv");
    ...
    //  Prune the grid to remove all background (0) values
    input_grid->pruneGrid(0.0f);  // Removes all inactive voxels with the background value of 0.0

    auto start = std::chrono::high_resolution_clock::now();
    // Apply the Lohou thinning algorithm (only on active voxels)
    openvdb::FloatGrid::Ptr thinned_grid = thinning_lohou_tbb(input_grid, templates);
    ...
    //  Save the output VDB file
    std::string output_filename = "C:/openvdb_drop/bin/thin_lohou_phys500.vdb";
    openvdb::io::File output_file(output_filename);
    openvdb::GridPtrVec grids;
    grids.push_back(thinned_grid);
